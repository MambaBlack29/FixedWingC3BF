...
At, P, Q, g, mu_e = sp.symbols('At P Q g mu_e', real = True)
...
# cbf with backstepping
h = sp.Matrix([sp.simplify((prel.norm() + 10*sp.simplify(((prel.T*vrel)[0,0])/prel.norm()) - rad - (((Rd - R)**2)/(2*mu_e))).subs(sub_dict_x))])
del_h = (h.jacobian(x))
